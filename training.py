...
def train():
  ...
  actor_learning_rate_fn = tf.optimizers.schedules.ExponentialDecay(FLAGS.actor_learning_rate, decay_steps=FLAGS.decay_steps, decay_rate=0.99, staircase=False)
  critic_learning_rate_fn = tf.optimizers.schedules.ExponentialDecay(FLAGS.critic_learning_rate, decay_steps=FLAGS.decay_steps, decay_rate=0.99, staircase=False)
    ...
    for batch_dataset in trans_dataset:
      with tf.GradientTape() as actor_tape, tf.GradientTape() as critic_tape:
        ...
        for item in batch_iterator:
          ...
          x, overdraft, r, z, critical, q = env(a, debit)
        ...
        #(t*a, f) --> (t*a)
        v = critic(s_batch, a_batch)

        #(t*a, f) --> (t*a)
        v_prime = critic(s_prime_batch, a_prime_batch)
